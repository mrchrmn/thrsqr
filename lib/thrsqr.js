...
function getNext(date) {
  ...
  let dstDifference = date.getUTCHours() - nextDate.getUTCHours();
  console.log("\n== dstDifference ==\n", dstDifference, "\n");
  return new Date(nextDate.valueOf() + (dstDifference * 60 * 60 * 1000));
}
...
function getResizedLogoURL(bucket, id, size) {
  const reqBody = {
    bucket,
    key: `logos/${id}-logo`,
    edits: {
  ...
  const url = `https://d2x3ofzbg5tm3i.cloudfront.net/${Buffer.from(JSON.stringify(reqBody)).toString("base64")}`;
